import pygame
import random
import math

# Initialize a game
pygame.init()

# create a screen
screen = pygame.display.set_mode((800, 600))

# title and icon
pygame.display.set_caption("Robert the Boss!")
icon = pygame.image.load('photo.jpg')
pygame.display.set_icon(icon)

# create player
playerimg = pygame.image.load('photo.jpg')
# change scale of the image
playerimg = pygame.transform.scale(playerimg, (60, 60))
playerX = 380
playerY = 480
playerX_change = 0

# create multiple enemies

enemyimg = []
enemyX = []
enemyY = []
enemyX_change = []
enemyY_change = []
num_enemies = 6

for i in range(num_enemies):
    enemyimg.append(pygame.image.load('enemy.png'))
    enemyX.append(random.randint(0, 735))
    enemyY.append(random.randint(50, 150))
    enemyX_change.append(3.5)
    enemyY_change.append(40)

# create bullet
bulletimg = pygame.image.load('bullet.png')

bulletX = 0
bulletY = 480
bulletX_change = 0
bulletY_change = 10
bullet_state = 'ready'

# create background
backgroundimg = pygame.image.load('backgroundimg.jpg')

# create score
score_value = 0
font = pygame.font.Font('freesansbold.ttf', 32)
textX = 10
textY = 10

#game over text
over_font = pygame.font.Font('freesansbold.ttf', 64)

def show_score(x, y):
    score = font.render('Score :' + str(score_value), True, (0, 0, 0))
    screen.blit(score, (round(x), round(y)))

def game_over_text():
    over_text = over_font.render('GAME OVER', True, (0, 0, 0))
    screen.blit(over_text, (200, 250))

def player(x, y):
    # draws a player on the screen
    screen.blit(playerimg, (round(x), round(y)))


def enemy(x, y, i):
    # draws an enemy on the screen
    screen.blit(enemyimg[i], (round(x), round(y)))


def fire_bullet(x, y):
    global bullet_state
    bullet_state = 'fire'
    screen.blit(bulletimg, (round(x + 10), round(y + 10)))


def isCollision(enemyX, enemyY, bulletX, bulletY):
    distance = math.sqrt((math.pow(enemyX - bulletX, 2)) + (math.pow(enemyY - bulletY, 2)))
    if distance < 27:
        return True
    else:
        return False


# game loop
running = True
while running:

    # RGB
    screen.fill((0, 0, 0))

    # add background img
    screen.blit(backgroundimg, (0, 0))
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False

        # if keystroke is pressed check if it's right or left (KEYDOWN - means pressing the key, KEYUP - releasing)
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                playerX_change = -5
            if event.key == pygame.K_RIGHT:
                playerX_change = 5
            if event.key == pygame.K_SPACE:
                if bullet_state == 'ready':
                    bulletX = playerX
                    fire_bullet(bulletX, bulletY)

        if event.type == pygame.KEYUP:
            if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:
                playerX_change = 0

    # checking for the boundaries of a player so he doesn't move out of the screen.
    playerX += playerX_change

    if playerX <= 0:
        playerX = 0
    elif playerX >= 750:
        playerX = 750

    # checking the boundaries of the enemy
    for i in range(num_enemies):

        #game over
        if enemyY[i] > 440:
            for j in range(num_enemies):
                enemyY[j] = 2000
            game_over_text()
            break

        enemyX[i] += enemyX_change[i]
        if enemyX[i] <= 0:
            enemyX_change[i] = 3.5
            enemyY[i] += enemyY_change[i]
        elif enemyX[i] >= 736:
            enemyX_change[i] = -3.5
            enemyY[i] += enemyY_change[i]

        # Collision
        collision = isCollision(enemyX[i], enemyY[i], bulletX, bulletY)
        if collision:
            bulletY = 480
            bullet_state = 'ready'
            score_value += 1
            enemyX[i] = random.randint(0, 735)
            enemyY[i] = random.randint(50, 150)
        enemy(enemyX[i], enemyY[i], i)

    # bullet movement
    if bulletY <= 0:
        bulletY = 480
        bullet_state = 'ready'

    if bullet_state == 'fire':
        fire_bullet(bulletX, bulletY)
        bulletY -= bulletY_change

    player(playerX, playerY)
    show_score(textX, textY)
    pygame.display.update()
